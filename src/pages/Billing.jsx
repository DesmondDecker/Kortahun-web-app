...
import {
  ...
  NumberInputField, NumberInputStepper, NumberIncrementStepper, Checkbox,
...
const emptyItem = { description: '', trips: 1, liters: 0, unitPrice: 0, totalPrice: 0 };
const emptyForm = {
...
async function generatePDF(bill) {
  ...
  doc.text('Na we dae take n Na we dae go lef', 14, 27); // Positioned directly below name
  ...
  const rows = (bill.lineItems || []).map(i => [
    i.description, i.trips, SLE(i.unitPrice), SLE(i.totalPrice),
  ]);

  autoTable(doc, {
    startY: 98,
    head: [['Description', 'Trips', 'Unit Price (SLE)', 'Total (SLE)']],
    body: rows,
    theme: 'striped',
  ...
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(8);
  doc.text(`Bank: ${bill.bankName || BANK_DETAILS.bank}`, 14, finalY + 5);
  doc.text(`Account Name: ${bill.accountName || BANK_DETAILS.accountName}`, 14, finalY + 10);
  doc.text(`Account Number: ${bill.accountNumber || BANK_DETAILS.accountNumber}`, 14, finalY + 15);
  doc.text(`SWIFT/BBAN: ${bill.swiftCode || BANK_DETAILS.swift}`, 14, finalY + 20);
...
function LineItemsEditor({ items, onChange }) {
  ...
  const update = (i, field, val) => {
    const next = items.map((item, idx) => {
      ...
      if (field === 'trips' || field === 'unitPrice') {
        updated.totalPrice = (parseFloat(updated.trips)||0) * (parseFloat(updated.unitPrice)||0);
      }
      return updated;
    });
    onChange(next);
        ...
        <Tbody>
          {items.map((item, i) => (
            <Tr key={i}>
              ...
              <Td>
                <NumberInput size="sm" min={0} value={item.trips}
                  onChange={v => update(i, 'trips', parseFloat(v)||0)}>
                  <NumberInputField />
              ...
              <Td fontSize="sm" fontWeight="semibold" color="brand.600">
                {SLE(item.totalPrice || (item.trips * item.unitPrice))}
              </Td>
              <Td>
                {items.length > 1 && (
...
function BillForm({ initial, customerList, onSave, onCancel, loading }) {
  ...
  const subtotal = (form.lineItems || []).reduce((s, i) => s + round((parseFloat(i.trips||0) * parseFloat(i.unitPrice||0))), 0);
  ...
  return (
    <VStack spacing={5} align="stretch">
      ...
      <Box p={3} bg="blue.50" borderRadius="md" border="1px" borderColor="blue.100">
        <Text fontSize="xs" fontWeight="bold" color="blue.700" mb={3}>BANK DETAILS FOR INVOICE</Text>
        <SimpleGrid columns={{ base: 1, md: 2 }} spacing={3}>
          <FormControl>
            <FormLabel fontSize="xs">Bank Name</FormLabel>
            <Input size="sm" bg="white" value={form.bankName} onChange={e => set('bankName', e.target.value)} />
          </FormControl>
          <FormControl>
            <FormLabel fontSize="xs">Account Name</FormLabel>
            <Input size="sm" bg="white" value={form.accountName} onChange={e => set('accountName', e.target.value)} />
          </FormControl>
          <FormControl>
            <FormLabel fontSize="xs">Account Number / BBAN</FormLabel>
            <Input size="sm" bg="white" value={form.accountNumber} onChange={e => set('accountNumber', e.target.value)} />
          </FormControl>
          <FormControl>
            <FormLabel fontSize="xs">Swift Code / Branch</FormLabel>
            <Input size="sm" bg="white" value={form.swiftCode} onChange={e => set('swiftCode', e.target.value)} />
          </FormControl>
        </SimpleGrid>
      </Box>
          ...
          onClick={() => {
            if (validate()) {
              ...
              toast({
                ...
                description: 'Please select a customer and ensure all line items have descriptions.',
                status: 'error',
                duration: 3000,
                isClosable: true,
                position: 'top'
...
export default function Billing() {
  ...
  const loadBills = useCallback(async () => {
    ...
    try {
      ...
      setSelectedIds([]); // Clear selection on refresh/page change
      setList(res.data || []);
  ...
  const handleSave = async (form) => {
    ...
    try {
      const data = { 
        ...
        bankName: form.bankName || BANK_DETAILS.bank,
        accountName: form.accountName || BANK_DETAILS.accountName,
        accountNumber: form.accountNumber || BANK_DETAILS.accountNumber,
        swiftCode: form.swiftCode || BANK_DETAILS.swift
      };
      // Fix: Check for a valid _id to distinguish between Update and Create
      if (selected && selected._id && isObjectId(selected._id)) {
  ...
  const handleBulkDeliveryNotes = async () => {
    setSaving(true);
    try {
      for (const id of selectedIds) {
        const res = await deliveryNotes.getForBill(id);
        await generateDeliveryNotePDF(res.data);
      }
      toast({ title: 'Success', description: `Generated ${selectedIds.length} delivery notes`, status: 'success' });
    } catch (e) {
      toast({ title: 'Bulk Error', description: e.message, status: 'error' });
    } finally {
      setSaving(false);
    }
  };
      ...
      <Flex justify="space-between" align="center" mb={6} wrap="wrap" gap={3}>
        ...
        <HStack spacing={3}>
          ...
          {selectedIds.length > 0 && (
            <Button colorScheme="teal" size="sm" leftIcon={<CheckIcon />} onClick={handleBulkDeliveryNotes} isLoading={saving}>
              Bulk Delivery Notes ({selectedIds.length})
            </Button>
          )}
      ...
      {at && (
        <SimpleGrid columns={{ base: 2, md: 5 }} spacing={3} mb={5}>
          {[
            ...
            { label:'Total Liters',  val: (at.totalLiters || 0).toLocaleString(), color:'orange', isCurr: false },
          ].map(s => (
            ...
            <Table>
              <Thead bg="gray.50">
                <Tr>
                  <Th w="40px">
                    <Checkbox 
                      isChecked={list.length > 0 && selectedIds.length === list.length}
                      isIndeterminate={selectedIds.length > 0 && selectedIds.length < list.length}
              ...
              <Tbody>
                {list.map(b => (
                  <Tr key={b._id} _hover={{ bg:'gray.50' }}>
                    <Td>
                      <Checkbox 
                        isChecked={selectedIds.includes(b._id)}
                    ...
                    <Td>
                      <HStack>
                        ...
                        <Tooltip label="Create Delivery Note">
                        ...
                        <Tooltip label="Edit">
                          <IconButton icon={<EditIcon />} size="sm" variant="ghost" colorScheme="brand"
                            onClick={() => {
                              // Ensure the customer field is an ID string, not an object or name string
        ...
        <ModalContent borderRadius="xl">
          ...
          <ModalBody>
            {viewing && (
              <VStack align="stretch" spacing={4}>
                ...
                <Table size="sm">
                  <Thead bg="gray.50">
                    <Tr><Th>Description</Th><Th isNumeric>Liters</Th><Th isNumeric>Unit</Th><Th isNumeric>Total</Th></Tr>
                  </Thead>
                  <Tbody>
                    {(viewing.lineItems||[]).map((item, i) => (
                      <Tr key={i}>
                        <Td>{item.description}</Td>
                        <Td isNumeric>{item.liters}</Td>
                ...
                <Box p={3} bg="blue.50" borderRadius="md" borderLeft="4px solid" borderColor="blue.400">
                  ...
                  <SimpleGrid columns={2} spacing={2} fontSize="xs">
                    <Text color="gray.600">Bank:</Text><Text fontWeight="semibold">{viewing.bankName || BANK_DETAILS.bank}</Text>
                    <Text color="gray.600">Account Name:</Text><Text fontWeight="semibold">{viewing.accountName || BANK_DETAILS.accountName}</Text>
                    <Text color="gray.600">Account Number:</Text><Text fontWeight="bold" color="blue.800">{viewing.accountNumber || BANK_DETAILS.accountNumber}</Text>
                    <Text color="gray.600">SWIFT/BBAN:</Text><Text fontWeight="semibold">{viewing.swiftCode || BANK_DETAILS.swift}</Text>
                  </SimpleGrid>
                </Box>
          ...
          <ModalFooter>
            <HStack>
              ...
              <Button colorScheme="teal" variant="outline" leftIcon={<CheckIcon />}
                onClick={async () => {
                  try {
                    const res = await deliveryNotes.getForBill(viewing._id);
                    generateDeliveryNotePDF(res.data);
                  } catch (e) {
                    toast({ title: 'Error', description: e.message, status: 'error' });
                  }
